...
async function getAccessToken(tenantId, clientId, clientSecret) {
  ...
  let parsed;
  try { parsed = JSON.parse(result.body); } catch(e) { parsed = {}; }
...
module.exports = async function (context, req) {
  ...
  try {
    ...
    if (action === 'testConnection') {
      ...
      let siteBody;
      try { siteBody = JSON.parse(siteResult.body); } catch(e) { siteBody = { raw: siteResult.body.slice(0,300) }; }
    ...
    if (action === 'getSiteId') {
      ...
      result = await graphRequest(token, 'POST',
        '/v1.0/sites/' + payload.siteId + '/drives/' + payload.driveId + '/root:/' + parentPath + ':/children',
      ...
      result = await graphRequest(token, 'GET',
        '/v1.0/sites/' + payload.siteId + '/drives/' + payload.driveId + '/root:/' + folderPath + ':/children?$orderby=name&$top=200', null);
      ...
      result = await new Promise((resolve, reject) => {
        const opts = {
          hostname: 'graph.microsoft.com',
          path: '/v1.0/sites/' + payload.siteId + '/drives/' + payload.driveId + '/root:/' + filePath + ':/content',
          method: 'PUT',
      ...
      result = await graphRequest(token, 'GET',
        '/v1.0/sites/' + payload.siteId + '/drives/' + payload.driveId + "/root/search(q='" + encodeURIComponent(payload.query) + "')?\$top=50", null);
    ...
    // Surface Graph errors clearly
    let parsed;
    try { parsed = JSON.parse(result.body); } catch(e) { parsed = { rawBody: result.body.slice(0,300) }; }
    ...
    if (result.status >= 400) {
      ...
      // Return a clean error the frontend can display
      context.res = { status: result.status, headers: {'Content-Type':'application/json'},
        body: JSON.stringify({ error: errMsg, graphStatus: result.status, action }) };
      return;
    }
